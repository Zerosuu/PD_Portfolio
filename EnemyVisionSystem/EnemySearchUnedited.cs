...
public class EnemySearch
{
    ...
    [SerializeField] private float _noiseDistanceQuiet = 0f, _noiseDistanceNormal = 5f, _noiseDistantLoud = 15f;
    ...
    public Vector3 NoiseRanges { get { return new Vector3(_noiseDistanceQuiet, _noiseDistanceNormal, _noiseDistantLoud); } }
    public Vector3 DetectionRadii { get { return new Vector3(_percentFar * _radius, _percentMid * _radius, _percentClose * _radius); } }
    private float[] DetectionSpeeds { get { return new float[] { _regressingSpeed, _speedFar, _speedMidFar, _speedCloseMid, _speedClose }; } }
    ...
    {
        if (MusicManager.Instace != null)
            AddListener(MusicManager.Instace);
    ...
    private IEnumerator FindPlayer()
    {
        ...
        while (_targetVState == TargetVisionState.Unseen)
        {
                ...
                || _enteredForbiddenArea)
            {
                ...
                switch (dist) // AlertSpeed based on % distance the player is from the enemy, based on it's radius. Sound detection may go beyond 100% when beyond radius
                {
    ...
    // Tracks if the player has been caught or not
    private IEnumerator ChasePlayer()
    {
        ...
        while (_targetVState == TargetVisionState.Seen) // VState variable changed in state machine
        {            
            ...
            { // Raycast may fail due to range being too short for difference in height, I.E. raycast goes downwards or upwards causing range to be incorrect
                ...
                if (!GameManager.Instance.LoadFailed)
                    ...
                    ScreenEffects.TriggerFadeOut(UnityEngine.SceneManagement.SceneManager.GetActiveScene().name);
            ...
            else if (Time.time - _timeLastSeen < 1f)
            {
                _posLastSeen = _targetTransform.position;
            }
    ...
    private float GetNoiseDistance()
    {
        return _playerNoiseLevel.Noiselvl switch
        {
            ...
            PlayerNoiseLevel.NoiseLevel.Loud => _noiseDistantLoud,
            _ => 0,
        };
    }
